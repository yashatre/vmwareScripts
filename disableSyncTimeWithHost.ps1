...
function removeSyncTimeWithHost($vmList) {
	try{
...
function setPolicyByDatacenter {
	try{
		...
		$datacenterVMList = Get-Datacenter $datacenter | Get-VM# | %{Get-View $_.ID} | where {$_.Guest.GuestFamily -match "windowsGuest"}
		removeSyncTimeWithHost($datacenterVMList)
	}
	catch{
...
function setPolicyByCluster {
	try{
		...
		$clusterVMList = Get-cluster $cluster | Get-VM# | %{Get-View $_.ID} | where {$_.Guest.GuestFamily -match "windowsGuest"}
		removeSyncTimeWithHost($clusterVMList)
	}
	catch{
		Write-Host -foregroundColor Red -backgroundColor Black "`nCould not execute the function..."
		throw $_.Exception.Message
